...
#pragma HLS PIPELINE off
    ...
    // Dictionary - reduced size and optimized access
    uintDictV_t dict[LZ_DICT_SIZE];
#pragma HLS BIND_STORAGE variable = dict type = RAM_1P impl = BRAM
    uintDictV_t resetValue = 0;
    for (int i = 0; i < MATCH_LEVEL; i++) {
#pragma HLS UNROLL
        resetValue.range((i + 1) * c_dictEleWidth - 1, i * c_dictEleWidth + MATCH_LEN * 8) = -1;
    }
// Optimized dictionary initialization with balanced resource usage
// Balanced parallelism for optimal speed without resource overflow
#ifndef DICT_UNROLL_FACTOR
#define DICT_UNROLL_FACTOR 8  // Balanced from 16 to 8 to prevent BRAM overflow
#endif

dict_flush:
    for (int i = 0; i < LZ_DICT_SIZE; i += DICT_UNROLL_FACTOR) {
#pragma HLS PIPELINE II = 1
#pragma HLS UNROLL FACTOR = 8
        // Balanced parallel dictionary initialization - 8 entries per cycle
        if (i < LZ_DICT_SIZE) dict[i] = resetValue;
        if (i+1 < LZ_DICT_SIZE) dict[i+1] = resetValue;
        if (i+2 < LZ_DICT_SIZE) dict[i+2] = resetValue;
        if (i+3 < LZ_DICT_SIZE) dict[i+3] = resetValue;
        if (i+4 < LZ_DICT_SIZE) dict[i+4] = resetValue;
        if (i+5 < LZ_DICT_SIZE) dict[i+5] = resetValue;
        if (i+6 < LZ_DICT_SIZE) dict[i+6] = resetValue;
        if (i+7 < LZ_DICT_SIZE) dict[i+7] = resetValue;
    }
lz_compress:
    for (uint32_t i = MATCH_LEN - 1; i < input_size - LEFT_BYTES && i < input_size; i++) {
#pragma HLS PIPELINE II = 1
#pragma HLS dependence variable = dict inter false
        uint32_t currIdx = i - MATCH_LEN + 1;
        ap_uint<8> next_byte = 0;
        if (i < input_size) {
...
#pragma HLS UNROLL
        ...
        present_window[MATCH_LEN - 1] = next_byte;
        ...
        // Ensure hash is within dictionary bounds
        hash = hash % LZ_DICT_SIZE;
...
// Optimized left bytes processing with balanced parallelism
// Balanced speed improvement without resource overflow
#ifndef LEFT_BYTES_UNROLL
#define LEFT_BYTES_UNROLL 8  // Balanced from 16 to 8 for resource efficiency
#endif

lz_left_bytes: {
    // Use balanced partitioning for good throughput without excess resources
    ap_uint<8> left_buffer[LEFT_BYTES];
#pragma HLS ARRAY_PARTITION variable=left_buffer cyclic factor=LEFT_BYTES_UNROLL
    
    // Fast read with balanced parallelism
read_left_bytes:
    for (int l = 0; l < LEFT_BYTES; l += LEFT_BYTES_UNROLL) {
#pragma HLS PIPELINE II = 1
#pragma HLS UNROLL FACTOR = LEFT_BYTES_UNROLL
        // Parallel read operations - each unrolled iteration processes 8 bytes
        for (int k = 0; k < LEFT_BYTES_UNROLL && (l + k) < LEFT_BYTES; k++) {
#pragma HLS UNROLL
            if ((l + k) < LEFT_BYTES) {
                left_buffer[l + k] = inStream.read();
            }
        }
    }
    
    // Fast processing with balanced parallelism
process_left_bytes:
    for (int l = 0; l < LEFT_BYTES; l += LEFT_BYTES_UNROLL) {
#pragma HLS PIPELINE II = 1
#pragma HLS UNROLL FACTOR = LEFT_BYTES_UNROLL
        // Parallel output operations - each unrolled iteration processes 8 bytes
        for (int k = 0; k < LEFT_BYTES_UNROLL && (l + k) < LEFT_BYTES; k++) {
#pragma HLS UNROLL
            if ((l + k) < LEFT_BYTES) {
                ap_uint<32> outValue = 0;
                outValue.range(7, 0) = left_buffer[l + k];
                outStream << outValue;
            }
        }
    }
...
void lzCompress(hls::stream<IntVectorStream_dt<8, 1> >& inStream, hls::stream<IntVectorStream_dt<32, 1> >& outStream) {
    ...
    uintDictV_t dict[LZ_DICT_SIZE];
#pragma HLS RESOURCE variable = dict core = RAM_1P
...
#pragma HLS UNROLL
        ...
        // Initialization of Dictionary - optimized for smaller dictionary
        dict_flush:
