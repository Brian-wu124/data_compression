...
#pragma HLS PIPELINE II = 1
        ...
        bool lit_count_overflow = (lit_count >= MAX_LIT_COUNT);
        if (lit_count_overflow) {
            lit_count_flag = 1;
        } else if (tLen) {
        ...
        // Optimize increment calculation
        i += tLen ? tLen : 1;
    }
    if (lit_count) {
        ap_uint<64> tmpValue;
...
#pragma HLS DEPENDENCE variable=match_length inter false
        ...
        // Optimized read operation with bounds check
        if (readOffsetFlag && (inIdx < input_size)) {
            nextLenOffsetValue = in_lenOffset_Stream.read();
            readOffsetFlag = false;
        }

        // Cache variables to reduce bit access delays
        ...
        if (next_state == WRITE_TOKEN) {
            ...
            // Consolidated end condition checks
            ...
            // Optimized token generation with ternary operators
            ap_uint<4> lit_token = (lit_length >= 15) ? (ap_uint<4>)15 : 
                                   (lit_length > 0) ? (ap_uint<4>)lit_length : (ap_uint<4>)0;
            outValue.range(7, 4) = lit_token;
            
            // Determine next state with simplified logic
            if (lit_length >= 15) {
                lit_length -= 15;
            ...
            // Match length token generation
            ap_uint<4> match_token = (match_length >= 15) ? (ap_uint<4>)15 : (ap_uint<4>)match_length;
            ...
            // Pre-compute offset+1 to reduce pipeline stalls
            match_offset_plus_one = match_offset + 1;
            
        } else if (next_state == WRITE_LIT_LEN) {
            ap_uint<8> len_val = (lit_length >= 255) ? (ap_uint<8>)255 : (ap_uint<8>)lit_length;
            outValue = len_val;
...
template <class data_t,
          ...
          int M_LEN = 6,
          int MIN_MAT = 4,
          int LZ_MAX_OFFSET_LIM = 32768,
          int OFFSET_WIN = 32768,
          int MAX_M_LEN = 255,
          int MAX_LIT_CNT = 2048,
          int MIN_B_SIZE = 128>
